...
通用爬虫通常会从一个种子URL地址开始，递归地搜索遍历该种子地址下的所有URL地址，聚焦式网络爬虫又称主题网络爬虫，会按照事先设定好的主题去爬取相关的页面。增量网络爬虫将参照已下载的网页内容，采用增量更新的方式只爬取没有下载或者已经发生变换的页面。深层网络爬虫主要用于爬取普通爬虫无法爬取到的深层网络，通常是提交某些参数（如登录操作）后才能到达的页面。
